...
from recognition.vietocr.model.transformerocr import OCR
from recognition.vietocr.model.vocab import Vocab
from recognition.vietocr.model.beam import Beam
...
def translate(img, model, max_seq_length=128, sos_token=1, eos_token=2):
    ...
    with torch.no_grad():
        ...
        while max_length <= max_seq_length and not all(np.any(np.asarray(translated_sentence).T == eos_token, axis=1)):
            ...
            #            output = model.transformer(ocr, tgt_inp, tgt_key_padding_mask=None)
            output, memory = model.transformer.forward_decoder(tgt_inp, memory)
            output = softmax(output, dim=-1)
            output = output.to('cpu')

            pred_max_prob, _ = output.max(dim=2)
            # pred_EOS = s.find('[s]')
            # pred = s[:pred_EOS]  # prune after "end of sentence" token ([s])
            # pred_max_prob_word = pred_max_prob[-1][:pred_EOS]
            # confidence_score = pred_max_prob_word.cumprod(dim=0)[-1]
        ...
        pred_max_prob_word = torch.tensor(char_probs).T[-1]
        conf = np.sum(pred_max_prob_word.tolist()) / len(pred_max_prob_word.tolist())
    ...
    return translated_sentence, conf
